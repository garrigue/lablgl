...
object(self)
  ...
  val rotate_posi = ( 0.2, -0.5, -5.5)
  val rotate_pos  = (-0.2,  0.5,  5.5)

  val id = 
    let id = GlList.create `compile in
    Glut.solidTorus 0.2 0.5 50 50; 
(*    Glut.solidTeapot 0.5; *)
    GlList.ends ();
    id

  method tick () = 
    angle <- mod_float (angle +. 0.5) 360.

  method render () =
    GlMat.push ();
    GlMat.rotate ~y:1.0 ~angle (); 
    GlMat.translate3 rotate_pos;
    GlMat.translate3 pos;
    GlMat.rotate ~y:(-1.0) ~angle ();
    GlMat.translate3 rotate_posi;
...
object(self)
  ...
  method tick () = 
    List.iter (fun teapot -> teapot#tick ()) teapots

  method render (jx,jy) (x,y,w,h) = 
    accPerspective viewport 45.0 ((float w) /. (float h)) (1.0, 15.0) (-1., 1.) (0.33*.jx,0.33*.jy) 5.0;
...
let main () =
  ...
  Glut.keyboardFunc ~cb:(fun ~key ~x ~y -> 
    match key with
      |	27(*esc*) -> exit 0
      | _ -> ());
  Glut.idleFunc(Some (fun () -> scene#tick (); scene#display ()));
