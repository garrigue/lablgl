(* $Id: glTex.ml,v 1.1 1998-01-29 11:46:02 garrigue Exp $ *)
...
let image1d img ?:proxy [< false >] ?:level [< 0 >]
    ?:internal [< format_size (format img) >] ?:border [< false >] =
...
let image2d img ?:proxy [< false >] ?:level [< 0 >]
    ?:internal [< format_size (format img) >] ?:border [< false >] =
...
type parameter =
    [ min_filter(filter) mag_filter([nearest linear])
      wrap_s(wrap) wrap_t(wrap) border_color(rgba)
      priority(clampf) ]
